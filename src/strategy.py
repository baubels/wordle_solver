...
def apply_strategy_random(guess_str: str, seed=42):
    """
    Apply a hand-made strategy for solving wordle puzzles. Works on random puzzles.
...
def apply_strategy_daily(guess_str: str):
    """
    Apply a hand-made strategy for solving wordle puzzles. Works on daily puzzle.
    ...
    # initial guess
    guess_made = eval(guess_daily(guess_str))
    ...
    while not guess_correct(guess_made):
        ...
        if len(remwords) == 1:
            print(f"Only remaining possibility: {remwords[0]}")
            guess_str = remwords[0]
            if not guess_correct(eval(guess_daily(guess_str))):
                print(f"Correct answer not contained in the dictionary. Woops.")
                break
            else:
                break
    ...
    was_guess_correct = guess_correct(eval(guess_daily(guess_str)))
    print(f"\nGuessing: '{guess_str}' ({was_guess_correct}) in {i+1} tries\n")
    return i + 1, was_guess_correct


apply_strategy_random('bashful', seed=12345)
